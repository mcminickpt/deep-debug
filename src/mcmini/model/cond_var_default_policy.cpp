...
void ConditionVariableDefaultPolicy::receive_broadcast_message() {
...
bool ConditionVariableDefaultPolicy::has_waiters() const {
...
bool ConditionVariableDefaultPolicy::thread_can_exit(runner_id_t tid) const {
  ...
  return std::any_of(wake_groups.begin(), wake_groups.end(),
                     [=](const WakeGroup &wg) { return wg.contains(tid); });
}

void ConditionVariableDefaultPolicy::wake_thread(runner_id_t tid) {
  ...
  if (this->broadcast_eligible_threads.count(tid) > 0) {
    ...
    const auto signal_to_consume =
        std::find_if(wake_groups.begin(), wake_groups.end(),
                     [=](const WakeGroup &wg) { return wg.contains(tid); });
